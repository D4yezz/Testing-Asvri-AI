...
test.describe("Pengaturan akun", () => {
  test("Ubah Profil (TC-01)", async ({ page }) => {
    ...
    await page
      .locator("div")
      .filter({ hasText: /^Ubah ProfilUbah Password$/ })
      .first()
      .click();
...
test.describe("Pindah Organisasi", () => {
  test("cari (TC-01)", async ({ page }) => {
    ...
    await page
      .getByRole("paragraph")
      .filter({ hasText: /^PKL JAGR$/ })
      .dblclick();
    await expect(page).toHaveURL(
      "https://sim.dev.asvri.ai/orgs/c2117f0f11f4a6aed00c4705f"
    );

    // belum tuntas karena organisasi pilihannya cunma 1
  });
});

test.describe("Keluar", () => {
  test("Keluar (TC-01)", async ({ page }) => {
    await page.goto("http://sim.dev.asvri.ai/");
    // await expect(page).toHaveURL('https://auth.dev.siap.id/sign-in');
    await page.getByRole("textbox").click();
    await page.getByRole("textbox").fill("PKLJAGR");
